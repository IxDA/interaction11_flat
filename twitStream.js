...
function fetch_tweets(elem){
	...
	$.getJSON(url,function(json){
		$(json.results).each(function(){
			...
			var tweetBy='<a class="tweet-user" target="_blank" href="http://twitter.com/'+this.from_user+'">'+this.from_user+' sez</a> ';
			//if(showTweetLinks.indexOf('reply')!=-1)
//				tweetBy=tweetBy+' &middot; <a class="tweet-reply" target="_blank" href="http://twitter.com/?status=@'+this.from_user+' &in_reply_to_status_id='+this.id+'&in_reply_to='+this.from_user+'">Reply</a>';  <span class="tweet-time">'+since+'</span>
...
//				tweetBy=tweetBy+' &middot; <a class="tweet-rt" target="_blank" href="http://twitter.com/?status=RT @'+this.from_user+' '+escape(this.text.replace(/&quot;/g,'"'))+'&in_reply_to_status_id='+this.id+'&in_reply_to='+this.from_user+'">RT</a>';
			var tweet='<div class="tweet"><div class="tweet-right">'+tweetBy+'<br>'+this.text.linkify().linkuser().linktag().replace(/<a/g,'<a target="_blank"')+'<br /><span class="tweet-time">'+since+'</span></div></div>';
			elem.append(tweet);
		});
	});
	return(false);
